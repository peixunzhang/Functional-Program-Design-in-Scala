...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(
      ...
      explored: Set[Block]
  ): LazyList[(Block, List[Move])] = neighbors.filterNot(nn => explored(nn._1))
  ...
  def from(
    ...
    val newExplored = newPaths.map(_._1).toSet
  ...
  lazy val solution: List[Move] =
    pathsToGoal.headOption.fold[List[Move]](Nil)(x => x._2.reverse)
